var main = (function(){
	...
	var questions = [
		...
		{count: 4, result: [], answer: [0,2,3]},
		{count: 4, result: [], answer: [0]},
		{count: 3, result: [], answer: [0,1,2]},
		{count: 4, result: [], answer: [2]},
	];
	return {
		init: function(){
			...
			$('section.game .qa img.a').css('height', (scrn.size.w * 0.375) + 'px');
			this.stats('visits');
			// $('#act1').on('vclick', function(e){
			// 	window.location.href = '../vip/index.html#7';
			// });
			$('section.game .qa img.a').css('height', (scrn.size.w * 0.375) + 'px');
			this.stats('visits');
