...
var app = {
	...
	init: function(data, praised){
		...
		$('.bar .focus').on('vclick', function(e){
			e.preventDefault();
			window.location.href = 'https://mp.weixin.qq.com/s?__biz=MjM5MzYxODY4NQ==&mid=400435029&idx=1&sn=4d62bfcde2ff814cf86a8a17a40a5281';
		});
	...
	onPhotoLoaded: function(){
        tpls.nickname = app.data.post.nickname;
        tpls.city = app.data.post.city;
		tpls.age = app.data.post.age;
	...
	adjustPraise: function(){
		...
		if (size > 0)
			$('.praise').css({width: size * 1.1, height: size * 1.1, 'line-height': (size * 1.2) + 'px'});
		else
