...
var app = {
	...
	init: function(){
		...
		for (var i = 0; i < this.opts.pageCount; i++) {
			var elm = $('.page:eq('+i+')').css({
				'background-image': 'url(img/' + (1 + i) + '.jpg)',
				'top': 0,
				'z-index': 100 - i,
			});
			i > 0 && elm.hide();
	...
	slideRestore: function(){
		...
		p1 && p1.animate({top: -size.h}, {
			easing: 'easeOutExpo', duration: 400
	 	});
		p2 && p2.animate({top: size.h}, {
			easing: 'easeOutExpo', duration: 400
	 	});
	},
	...
	startTitleAnimations: function(){
		...
		var span = 2500;
		es[0].animate({opacity: 1}, {
         	easing: 'easeOutQuad',
         	duration: span,
	 	});
	 	var t = 0, inv = 700; span = 1000;
	 	app.titles.curIdx = 0;
	 	for (var i = 1; i < es.length; i++) {
		 	es[i].css({left: ps[i].from});
	 		setTimeout(function(){
	 			var idx = ++app.titles.curIdx;
			 	es[idx].animate({left: ps[idx].to, opacity:1}, {
