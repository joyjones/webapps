...
var stats = function(key){
	$.ajax({
		url: '/lib/stats.php',
		data: {key: 'questions-' + key, addval: 1},
		dataType: 'json'
	});
};
...
var submit = function(){
	...
	if (phone && phone.length > 0 && !/^1[3|4|5|8][0-9]\d{4,8}$/.test(phone)){
		alert('请填写您有效的手机号码。'); return;
	}
	if (!email || !/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/.test(email)) {
		alert('请填写您有效的电子邮件地址。'); return;
	}
	if (prov < 0){
		alert('请选择您所在的城市。'); return;
	}
	if (industry < 0){
		alert('请选择您所在的行业。'); return;
	}
	$.ajax({
		url: 'submit.php',
		type: 'post',
		data: {
			...
			corp: corp,
			position: position,
			industry: configs.industries[industry],
...
$(function(){
	...
	for (var i = 0; i < configs.industries.length; i++) {
		$('#industry').append('<option value="'+i+'">'+configs.industries[i]+'</option>');
	};
	$('img#submit').on('vclick', function(){
		submit();
	});

	weixin.init({
		title: '线上互动问题收集',
		desc: '积极参与者有机会获得商论送出的移动充电宝！',
		imgUrl: 'http://tegbr.sinaapp.com/questions/img/logo.jpg',
		success: function() {
			stats('sharePage');
		}
	});
